...
if hotel_matrix(i,j-1,f) > 0 %room(i,j-1,f).num > 0 / would be the same 
    ...
    if room(i,j,f).wall1(7) == 1
        q1_sun = solar_rad_norm(v-1)*Area1*t;    
    elseif room(i,j,f).wall1(7) == 2
        q1_sun = solar_rad_normW(v-1)*Area1*t;
    elseif room(i,j,f).wall1(7) == 4
        q1_sun = solar_rad_normE(v-1)*Area1*t;
    else
        q1_sun = 0;
    end
...
if hotel_matrix(i-1,j,f) > 0
    ...
    if room(i,j,f).wall2(7) == 1
        q2_sun = solar_rad_norm(v-1)*Area2*t;    
    elseif room(i,j,f).wall2(7) == 2
        q2_sun = solar_rad_normW(v-1)*Area2*t;
    elseif room(i,j,f).wall2(7) == 4
        q2_sun = solar_rad_normE(v-1)*Area2*t;
    else
        q2_sun = 0;
    end
...
if hotel_matrix(i,j+1,f) > 0
    ...
    if room(i,j,f).wall3(7) == 1
        q3_sun = solar_rad_norm(v-1)*Area3*t;    
    elseif room(i,j,f).wall3(7) == 2
        q3_sun = solar_rad_normW(v-1)*Area3*t;
    elseif room(i,j,f).wall3(7) == 4
        q3_sun = solar_rad_normE(v-1)*Area3*t;
    else
        q3_sun = 0;
    end
...
if hotel_matrix(i+1,j,f) > 0
    ...
    if room(i,j,f).wall4(7) == 1
        q4_sun = solar_rad_norm(v-1)*Area4*t;    
    elseif room(i,j,f).wall4(7) == 2
        q4_sun = solar_rad_normW(v-1)*Area4*t;
    elseif room(i,j,f).wall4(7) == 4
        q4_sun = solar_rad_normE(v-1)*Area4*t;
    else
        q4_sun = 0;
    end
...
if hotel_matrix(i,j,f-1) > 0
    ...
    if room(i,j,f).wall5(7) == 1
        q5_sun = solar_rad_norm(v-1)*Area5*t;    
    elseif room(i,j,f).wall5(7) == 2
        q5_sun = solar_rad_normW(v-1)*Area5*t;
    elseif room(i,j,f).wall5(7) == 4
        q5_sun = solar_rad_normE(v-1)*Area5*t;
    else
        q5_sun = 0;
    end
