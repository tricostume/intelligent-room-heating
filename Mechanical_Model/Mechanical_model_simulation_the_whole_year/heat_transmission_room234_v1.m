...
%   I substitute all the v-1 with steps because I am using the last value
%   of the prevuoius day
%   For the sun I used the last value of the same day cuz it is easier,
%   even if it is not so properly (the error is really low)
...
Tinit_air = room_temp(i,j,f).temperature(steps);
...
for u = 1 : 4
    if hotel_matrix(i+g1(u),j+g2(u),f) == 0 || hotel_matrix(i+g1(u),j+g2(u),f) == 1
        Textern = room_temp(i+g1(u),j+g2(u),f).temperature(steps);
...
if room(i,j,f).wall1(7) == 1 %solar radiation window1
    q1_sun = solar_rad_norm(steps)*Area1w*t;    
elseif room(i,j,f).wall1(7) == 2
    q1_sun = solar_rad_normW(steps)*Area1w*t;
elseif room(i,j,f).wall1(7) == 4
    q1_sun = solar_rad_normE(steps)*Area1w*t;
else
    q1_sun = 0;
end
if room(i,j,f).wall2(7) == 1 %solar radiation window2
    q2_sun = solar_rad_norm(steps)*Area2w*t;    
elseif room(i,j,f).wall2(7) == 2
    q2_sun = solar_rad_normW(steps)*Area2w*t;
elseif room(i,j,f).wall2(7) == 4
    q2_sun = solar_rad_normE(steps)*Area2w*t;
else
    q2_sun = 0;
end
if room(i,j,f).wall3(7) == 1 %solar radiation window3
    q3_sun = solar_rad_norm(steps)*Area3w*t;    
elseif room(i,j,f).wall3(7) == 2
    q3_sun = solar_rad_normW(steps)*Area3w*t;
elseif room(i,j,f).wall3(7) == 4
    q3_sun = solar_rad_normE(steps)*Area3w*t;
else
    q3_sun = 0;
end
if room(i,j,f).wall4(7) == 1 %solar radiation window4
    q4_sun = solar_rad_norm(steps)*Area4w*t;    
elseif room(i,j,f).wall4(7) == 2
    q4_sun = solar_rad_normW(steps)*Area4w*t;
elseif room(i,j,f).wall4(7) == 4
    q4_sun = solar_rad_normE(steps)*Area4w*t;
else
    q4_sun = 0;
end
...
if room_temp(i,j,f).book(steps) ~= 0
    gapT = 1.5;
    if real_step == 1
        room(i,j,f).extremeT(1) = Tlow + gapT; room(i,j,f).extremeT(2) = Thigh + gapT; %initialization
    end

    %heating part gap plus/mois 1
    if room_temp(i,j,f).temperature(steps) < Tlow - gapT
    ...
    if room_temp(i,j,f).temperature(steps) < Thigh - gapT
    ...
    if room_temp(i,j,f).temperature(steps) < room(i,j,f).extremeT(1) %Tlow_true
        qH = qHeater*room_temp(i,j,f).book(steps); qC = 0;
    elseif room_temp(i,j,f).temperature(steps) > room(i,j,f).extremeT(2) %Thigh_true
        qH = 0; qC = qCooler*room_temp(i,j,f).book(steps);
...
% if i == 3 && j == 5 && f == 3
% fprintf('%f\t %f\t %f\t %f\t %f\t %f\t %f\t %f\n',f,i,j,v,room(i,j,f).temperature(v)-C2K,qH,qC,pointH);
% end
